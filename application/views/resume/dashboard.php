<div class="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
    ...
    <div id="ra-stats" class="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <a href="<?= base_url('resume/create/1'); ?>" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-brand-600 to-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-600/30 transition hover:-translate-y-0.5 hover:from-brand-500 hover:to-indigo-500">
    ...
    <div class="mt-10 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <div class="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-lg shadow-slate-200/40 ring-1 ring-slate-100 transition duration-300 hover:-translate-y-1 hover:shadow-xl dark:border-slate-700 dark:bg-slate-900 dark:shadow-none dark:ring-slate-800">
            <div class="absolute -right-4 -top-4 h-20 w-20 rounded-full bg-gradient-to-br from-violet-200/60 to-transparent opacity-0 transition group-hover:opacity-100 dark:from-violet-600/20"></div>
        ...
        <div class="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-lg shadow-slate-200/40 ring-1 ring-slate-100 transition duration-300 hover:-translate-y-1 hover:shadow-xl dark:border-slate-700 dark:bg-slate-900 dark:shadow-none dark:ring-slate-800">
            <div class="absolute -right-4 -top-4 h-20 w-20 rounded-full bg-gradient-to-br from-sky-200/60 to-transparent opacity-0 transition group-hover:opacity-100 dark:from-sky-600/20"></div>
    ...
    <section id="your-resumes" class="mt-14 scroll-mt-28">
        ...
        <?php if (empty($resumes)): ?>
            <div class="rounded-3xl border-2 border-dashed border-slate-200 bg-white/80 p-12 text-center shadow-inner dark:border-slate-700 dark:bg-slate-900/50">
                <div class="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-brand-500 to-indigo-600 text-white shadow-lg shadow-brand-500/30">
            ...
            <div class="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
                ...
                ?>
                    <article class="group flex flex-col overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-lg shadow-slate-200/50 ring-1 ring-slate-100 transition duration-300 hover:-translate-y-1 hover:border-brand-200/80 hover:shadow-2xl hover:shadow-brand-500/10 dark:border-slate-700 dark:bg-slate-900 dark:shadow-none dark:ring-slate-800 dark:hover:border-brand-500/40">
                        <div class="relative aspect-[1/1.414] overflow-hidden bg-gradient-to-br from-slate-100 via-white to-slate-50 dark:from-slate-800 dark:via-slate-900 dark:to-slate-800">
                            <div class="absolute inset-5 rounded-lg bg-white shadow-md ring-1 ring-slate-100 dark:bg-slate-950 dark:ring-slate-800">
                                <div class="flex h-full flex-col gap-1.5 p-4">
                                    <div class="h-2.5 w-2/3 rounded-md bg-gradient-to-r from-slate-800 to-slate-600 dark:from-slate-200 dark:to-slate-400"></div>
                                    <div class="h-1.5 w-1/2 rounded-md bg-slate-300 dark:bg-slate-600"></div>
                                    <div class="mt-3 space-y-1">
                                        <div class="h-1 w-full rounded bg-slate-200 dark:bg-slate-700"></div>
                                        <div class="h-1 w-5/6 rounded bg-slate-200 dark:bg-slate-700"></div>
                                        <div class="h-1 w-4/6 rounded bg-slate-200 dark:bg-slate-700"></div>
                                    </div>
                                    <div class="mt-3 h-1.5 w-1/3 rounded-md bg-gradient-to-r from-brand-600 to-indigo-500"></div>
                                    <div class="mt-2 space-y-1">
                                        <div class="h-1 w-full rounded bg-slate-200 dark:bg-slate-700"></div>
                                        <div class="h-1 w-3/4 rounded bg-slate-200 dark:bg-slate-700"></div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex flex-1 flex-col border-t border-slate-100 p-5 dark:border-slate-800">
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                                    <div class="h-full rounded-full bg-gradient-to-r from-emerald-400 to-brand-500 transition-all duration-500" style="width: <?= $bar; ?>%"></div>
                                </div>
                            </div>
                            <div class="mt-5 flex gap-2">
                                <a href="<?= base_url('resume/edit/' . $r->id); ?>" class="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-brand-600 to-indigo-600 px-3 py-2.5 text-sm font-semibold text-white shadow-md shadow-brand-600/20 transition hover:from-brand-500 hover:to-indigo-500">Edit</a>
        ...
        <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <?php foreach ($templates as $t): ?>
                <a href="<?= base_url('resume/create/' . $t->id); ?>" class="group overflow-hidden rounded-2xl border border-slate-200/90 bg-white p-5 shadow-md shadow-slate-200/40 ring-1 ring-slate-100 transition duration-300 hover:-translate-y-1 hover:border-brand-200 hover:shadow-xl dark:border-slate-700 dark:bg-slate-900 dark:shadow-none dark:ring-slate-800">
                    <div class="aspect-[1/1.414] rounded-xl bg-gradient-to-br from-slate-100 to-slate-50 ring-1 ring-slate-200/80 transition group-hover:from-brand-50 group-hover:to-indigo-50 dark:from-slate-800 dark:to-slate-900 dark:ring-slate-700"></div>
