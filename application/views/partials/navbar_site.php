...
<nav class="sticky top-0 w-full bg-white/90 backdrop-blur-md border-b border-gray-100 z-50 transition-all duration-300">
    <div class="max-w-[1400px] mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <a href="<?= base_url(); ?>" class="flex items-center gap-2 group shrink-0">
            <img src="<?= base_url('assets/website/images/previews/internmo.jpeg'); ?>" alt="Internmo Logo" class="h-11 w-auto object-contain group-hover:opacity-90 transition-all">
        </a>

        <!-- Center Menu (desktop) -->
        <div class="hidden lg:flex items-center gap-6" x-data>
            <a href="<?= base_url(); ?>" class="text-sm font-<?= $is_home ? 'semibold text-amber-500' : 'medium text-slate-600 hover:text-amber-500'; ?> transition-colors relative <?= $is_home ? "after:absolute after:bottom-0 after:left-0 after:w-full after:h-[2px] after:bg-amber-500" : "after:absolute after:-bottom-1 after:left-0 after:w-0 after:h-[2px] after:bg-amber-500 after:transition-all hover:after:w-full"; ?>">Home</a>

            <!-- Courses Dropdown -->
            <div class="relative" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                <button @click="open = !open" class="text-sm font-medium text-slate-600 hover:text-amber-500 transition-colors py-2 flex items-center gap-1">
                    Courses <i class="fa-solid fa-chevron-down text-[8px] transition-transform duration-200" :class="open ? 'rotate-180 text-amber-500' : ''"></i>
                </button>
                <!-- Mega Menu -->
                <div x-show="open"
