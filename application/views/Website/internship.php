...
<?php endif; ?>
<div class="relative overflow-hidden bg-gradient-to-b from-indigo-50/80 to-white">
    <div class="absolute inset-x-0 top-0 h-64 bg-gradient-to-b from-indigo-100/50 to-transparent pointer-events-none"></div>
    ...
    <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
        ...
        <div class="ra-animate-in lg:flex lg:items-end lg:justify-between gap-8">
            <div class="max-w-2xl">
                <p class="text-sm font-semibold uppercase tracking-wider text-indigo-600">Learning</p>
            ...
            <div class="ra-animate-in ra-animate-in-delay-2 mt-6 lg:mt-0">
                <div class="rounded-2xl bg-indigo-600 text-white px-5 py-3 shadow-lg ra-glow-pulse">
        ...
        <form method="get" action="<?= base_url('internship'); ?>"
            ...
            <div class="relative flex-1">
                ...
                <input type="search" name="q" value="<?= htmlspecialchars($q); ?>"
                       ...
                       class="w-full pl-10 pr-3 py-2.5 rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500">
            </div>
            <select name="category" class="rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 py-2.5">
            ...
            <label class="inline-flex items-center gap-2 text-sm text-slate-700 px-2">
                <input type="checkbox" name="remote" value="1" <?= $remote ? 'checked' : ''; ?> class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                Remote only
            </label>
            <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-5 py-2.5 transition">Filter</button>
        </form>

        <!-- Results -->
        <?php if (empty($internships)): ?>
            <div class="mt-12 rounded-3xl border-2 border-dashed border-slate-200 bg-white px-8 py-16 text-center">
                <div class="mx-auto w-14 h-14 grid place-items-center rounded-xl bg-indigo-50 text-indigo-700">
                ...
                <a href="<?= base_url('internship'); ?>" class="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-indigo-700 hover:text-indigo-900">Clear filters →</a>
            </div>
        <?php else: ?>
            <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                ?>
                    <article class="ra-animate-in ra-animate-in-delay-<?= min($idx++, 4); ?> group relative rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-indigo-200 transition-all duration-300">
                        ...
                        <div class="flex items-start gap-3 pr-10">
                            ...
                            <div class="min-w-0 flex-1">
                                <h2 class="font-bold text-slate-900 truncate group-hover:text-indigo-700 transition"><?= htmlspecialchars($i->title); ?></h2>
                                <p class="text-sm text-slate-500 truncate"><?= htmlspecialchars($i->company); ?></p>
                            </div>
                            <span class="shrink-0 text-[11px] font-semibold rounded-full px-2 py-0.5 bg-indigo-100 text-indigo-800">
                        ...
                        <div class="mt-4 flex flex-wrap gap-2 text-xs text-slate-600">
                            ...
                            <?php if (!empty($i->duration_weeks)): ?>
                                <span class="inline-flex items-center gap-1 rounded-full bg-violet-50 text-violet-700 px-2.5 py-1 font-medium">
                        ...
                        <a href="<?= $this->session->userdata('logged_in') ? base_url('dashboard') : base_url('register'); ?>"
                           class="mt-5 inline-flex items-center gap-2 text-sm font-bold text-indigo-600 hover:text-indigo-800">
                            Apply now
        ...
        <div class="ra-animate-in ra-animate-in-delay-4 mt-14 rounded-2xl bg-slate-900 text-white p-8 sm:p-10">
            ...
            <a href="<?= base_url('project-submission'); ?>" class="mt-6 inline-flex rounded-xl bg-white text-slate-900 font-bold px-5 py-3 hover:bg-indigo-100 transition">
                Submit application
            </a>
        </div>
    </div>
